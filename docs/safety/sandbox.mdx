...
> “如果它连 `/etc/hosts` 都不让我改，那要它干嘛？”

答案是：它本来就不是为了让 shell 随便改系统目录，而是为了让 Claude Code 可以更放心地运行那些 **本不应该碰系统目录** 的命令。
...
这些命令大多应该只在工作区和少量临时目录里活动。沙箱把能力边界收缩到这个范围后，项目才敢在应用层减少弹窗、启用 auto-allow、提高自动化程度。

换句话说：

- 沙箱不是为了帮你改 `/etc`
- 沙箱是为了帮你放心地跑那些“不该改 `/etc` 的命令”
