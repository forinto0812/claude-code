...
```ts
isVoiceModeEnabled() = hasAvailableVoiceProvider() && isVoiceGrowthBookEnabled()
```
...
3. **Provider 可用性检查**：`hasAvailableVoiceProvider()` — 需要可用的 STT provider（Claude.ai OAuth 或受支持的 API key provider）
