...
Claude Code 定义了 27 种 Hook 事件（`src/entrypoints/sdk/coreTypes.ts` 中 `HOOK_EVENTS` 数组），覆盖完整的 Agent 生命周期：
...
Hooks 配置支持 6 种执行方式，定义分布在三个文件中：
- `command`、`prompt`、`http`、`agent` — `src/schemas/hooks.ts` 中的 Zod schema
- `callback` — `src/types/hooks.ts` 中的 `HookCallback` 类型
- `function` — `src/utils/hooks/sessionHooks.ts` 中的 `FunctionHook` 类型
...
`execCommandHook()`（`src/utils/hooks.ts`，命令型 Hook 执行核心函数）是命令型 Hook 的执行核心：
...
Hook 进程的 stdout 第一行如果是 `{"async":true}`，系统将其转为后台任务（`src/utils/hooks.ts` 中异步检测逻辑）：
...
同步 Hook 的输出遵循严格的 Zod schema（`src/types/hooks.ts` 中 `syncHookResponseSchema` 及各事件的 `hookSpecificOutput` schema）：
...
`getMatchingHooks()`（`src/utils/hooks.ts`，Hook 匹配主函数）负责从所有来源中查找匹配的 Hook：
...
`matcher` 字段支持三种模式（`matchesPattern()`，`src/utils/hooks.ts`）：
...
Hook 可以指定 `if` 条件，只在特定输入时触发。`prepareIfConditionMatcher()`（`src/utils/hooks.ts`）预编译匹配器：
...
同一个 Hook 命令在不同配置层级（user/project/local）可能重复。系统按复合键做 Map 去重，保留**最后合并的层级**。去重键格式为 `{pluginRoot|skillRoot}\0{shell}\0{command}\0{ifCondition}`（四部分通过 null 字节连接），不仅限于命令文本。

## 工作区信任检查

**所有 Hook 都要求工作区信任**（`shouldSkipHookDueToTrust()`，`src/utils/hooks.ts`）。这是纵深防御措施——防止恶意仓库的 `.claude/settings.json` 在未信任的情况下执行任意命令。
...
Agent 和 Skill 的前置 Hook 通过 `registerFrontmatterHooks()`（定义在 `src/utils/hooks/registerFrontmatterHooks.ts`，在 `runAgent.ts` 中调用）注册，绑定到 agent 的 session ID。Agent 结束时通过 `clearSessionHooks()`（定义在 `src/utils/hooks/sessionHooks.ts`，`runAgent.ts` finally 块中调用）清理。

```typescript
// runAgent.ts — 注册 agent 的前置 Hook（调用点）
registerFrontmatterHooks(rootSetAppState, agentId, agentDefinition.hooks, ...)

// runAgent.ts — finally 块清理（调用点）
