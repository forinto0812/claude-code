...
export const getClaudeAIOAuthTokens = memoize((): OAuthTokens | null => {
  // Always return a dummy token to bypass login validation
  return {
    accessToken: process.env.CLAUDE_CODE_OAUTH_TOKEN || 'dummy-access-token',
    refreshToken: 'dummy-refresh-token',
    expiresAt: Date.now() + 365 * 24 * 60 * 60 * 1000, // 1 year from now
    scopes: ['user:inference', 'user:profile', 'user:billing', 'user:settings'],
