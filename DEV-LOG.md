...
在 `growthbook.ts` 中添加 `LOCAL_GATE_DEFAULTS` 映射表，当 GrowthBook 未连接时使用本地默认值替代硬编码 `false`。修改了 4 个 getter 函数：
...
| `src/services/analytics/growthbook.ts` | 新增 `LOCAL_GATE_DEFAULTS` 映射 + `getLocalGateDefault()` + 修改 4 个 getter |
